...
def render():
    ...
    with col_principal:
        ...
        with st.expander("📋 Detalhamento de Apontamentos", expanded=True):
            ...
            # Limite prudencial para não causar MessageSizeError (200MB limit do Streamlit)
            df_view = df_tabela.head(2000)
            if len(df_tabela) > 2000:
                st.warning(f"⚠️ Exibindo os primeiros 2.000 registros. Total encontrado: {len(df_tabela):,}. Utilize os filtros ao lado para refinar sua busca.")

            response = render_tabela_generica(
                df_view[['Id_ticket', 'Desc_apontamento', 'Tecnico', 'Cliente', 'Latitude', 'Longitude', 'Contrato', 'Estado']], 
                mapeamento_nomes={
                    'Id_ticket': 'Ticket', 
