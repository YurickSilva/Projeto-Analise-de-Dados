...
def render():
    ...
    with col_filtros:
        ...
        data_ini, data_fim = render_filtro_periodo(
            df_raw, "criado_em", "geral_periodo", titulo_expander="📅 Período de Análise"
        )
    ...
    with col_principal:
        ...
        with c1:
            with st.expander("Total de Tickets", expanded=True):
        ...
        with g1:
            with st.expander("Teste de Avaliação", expanded=True):
                if "avaliacao" in df.columns and not df.empty:
                    df_aval = df.groupby("avaliacao").size().reset_index(name="contagem")
                    render_grafico_barras_ranking(df_aval, "avaliacao", "contagem")
                else:
                    st.info("Coluna de avaliação não disponível.")

        with g2:
            with st.expander("Evolução por Ano", expanded=True):
                if "fechado_em" in df.columns and not df.empty:
                    df_local = df.copy()
                    df_local["ano"] = pd.to_datetime(df_local["fechado_em"], errors='coerce').dt.year
                    df_ano = df_local.dropna(subset=["ano"]).groupby("ano").size().reset_index(name="contagem")
                    if not df_ano.empty:
        ...
        with m1:
            with st.expander("SLA Atendido", expanded=True):
        ...
        with m2:
            with st.expander("Tickets por Mesa", expanded=True):
                if "mesa" in df.columns and "cliente" in df.columns and not df.empty:
                    df_mesa = df.groupby("mesa")["cliente"].nunique().reset_index(name="total_clientes")
