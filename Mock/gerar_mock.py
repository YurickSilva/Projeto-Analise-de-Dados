...
def gerar_mock_final(n_clientes=400, n_tickets=300000, valor_hora=150.0):
    ...
    n_normal_users = max(1, n_clientes - n_heavy_users)
    pesos = [(0.8 / n_heavy_users) if i < n_heavy_users else (0.2 / n_normal_users) for i in range(n_clientes)]

    # --- 4. TABELA CLIENTES ---
    df_clientes = pd.DataFrame([{
        "Id_cliente": c['id'], "Cliente": c['nome'], "Razao_social": c['razao'], "Situacao": random.choice(["Ativo", "Inativo"]),
        "Criado_em": fake.date_time_between(start_date='-7y').strftime('%Y-%m-%d %H:%M:%S.%f'),
    ...
    tickets_data, apontamentos_data, extras_data = [], [], []
    id_ap_counter, id_ex_counter = 200000, 300000
    data_inicio_2020 = datetime(2020, 1, 1)
    ...
    for i in range(n_tickets):
        ...
        criado_dt = fake.date_time_between(start_date=data_inicio_2020, end_date='now')
        ...
        if "On Site" in mesa_atual and random.random() < 0.4:
            for _ in range(random.randint(1, 3)):
                ...
                extras_data.append({
                    "Id_ticket": id_t, "Mesa": mesa_atual, "Data_apontamento": criado_dt.strftime('%Y-%m-%d %H:%M:%S'),
                    "Responsavel": tecnico_resp, "Valor_extra": ex['tipo'], "Desc_valorizacao": ex['desc'],
                    "Quantidade": float(qtd), "Valor_unitario": ex['unitario'], 
                    "Valorizacao_extra": round(qtd * ex['unitario'], 2),
                    "Custo_Interno": random.choice(["Sim", "Não"]), "Id_valorizacao": id_ex_counter
                })
                id_ex_counter += 1
        ...
        tickets_data.append({
            ...
            "Desc_ticket": limpar_texto(fake.sentence(nb_words=5)), "Situacao": random.choice(["Fechado", "Aberto"]),
            "Mesa": mesa_atual, "Responsavel": tecnico_resp, "Solicitado_por": solicitante['nome'], 
            "Email_responsavel": solicitante['email'], "Ultima_resposta": random.choice(["Cliente", "Interno", "Nenhum"]),
            ...
            "Avaliacao": float(random.randint(1, 5)), "SLA_atendido": random.choice(["Sim", "Não", "Sem SLA"]),
            "Duracao_apontada": duracao_total_ticket, "Tempo_total_fechamento": round(duracao_total_ticket / 60, 4),
...
# Salvando os arquivos dentro da pasta definida
df_c.to_csv(os.path.join(pasta_destino, "Tiflux_tb_Clientes_MOCK.csv"), index=False, encoding='utf-8-sig')
df_t.to_csv(os.path.join(pasta_destino, "Tiflux_tb_Tickets_MOCK.csv"), index=False, encoding='utf-8-sig')
df_a.to_csv(os.path.join(pasta_destino, "Tiflux_tb_Apontamentos_MOCK.csv"), index=False, encoding='utf-8-sig')
df_e.to_csv(os.path.join(pasta_destino, "Tiflux_tb_Valores_Extras_MOCK.csv"), index=False, encoding='utf-8-sig')

print(f"Sucesso!")
print(f"Arquivos salvos em: {os.path.abspath(pasta_destino)}")
print(f"- Periodo: {df_t['Criado_em'].min()[:10]} ate {df_t['Criado_em'].max()[:10]}")
